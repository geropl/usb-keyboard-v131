...
//  
int max_pin = 28; // the keyboard FPC connector pin count. If set to 34, unsolder the LED or the code won't work
int min_pin = 2; // the first pin to be tested on the FPC connector (usually pin 1)
//
...
//
  for (int i=0; i<max_pin-1; i++) {   // outer loop pin
    ...
    for (int j=i+1; j<max_pin; j++) {   // inner loop pin
      delayMicroseconds(20); // give time to let the signals settle out
